----------------------------------------------------------------------
-- Package Utilities.Timing (body)                                  --
-- (C) Copyright 1999, 2013 ADALOG                                  --
-- Author: J-P. Rosen                                               --
--                                                                  --
...
package body Utilities.Timing is
   use Ada.Calendar, Ada.Strings.Wide_Unbounded;
   ...
   function Timer_Header (The_Timer : Timer_Index) return Wide_String is
      ...
      Common_Part : constant Wide_String := Timer_Name & Timer_Index'Wide_Image(The_Timer);
   begin
   ...
   begin
      if Timer_Obj.Nb_Calls = 0 then
         ...
         Trace (Timer_Header (The_Timer)
                & Total_Message    & Duration'Wide_Image (Timer_Obj.Total) & Seconds_Message
                & Nb_Calls_Message & Integer 'Wide_Image (Timer_Obj.Nb_Calls)
                & Average_Message  & Duration'Wide_Image (Timer_Obj.Total/Timer_Obj.Nb_Calls)
                & Seconds_Message);
      end if;
   exception
      when Occur : others =>
         Failure ("Timing.Report", Occur);
   end Report;
   ...
   -- Make sure timings are reported at the end of Debug.
   ...
   procedure Finalize (Object : in out Proxy);

   procedure Finalize (Object : in out Proxy) is
      pragma Unreferenced (Object);
   begin
      Trace (Last_Timer_Message);
      Report_All;
   end Finalize;
   Dummy : Proxy;
   pragma Unreferenced (Dummy);
end Utilities.Timing;
